...
export function AdmitCardPanel({
  ...
  async function loadPrintData() {
    try {
      ...
      const [studentPage, docs] = await Promise.all([
        ...
        apiRequest<GeneratedAdmit[]>("/documents/generated?type=ADMIT_CARD", token),
      ]);
  ...
  async function generateAdmit(event: FormEvent) {
    event.preventDefault();
    ...
    setGenerating(true);
    try {
      const doc = await apiRequest<GeneratedAdmit>("/documents/generated", token, {
        method: "POST",
        body: JSON.stringify({
      ...
      notifySuccess("Admit card generated");
      await loadPrintData();
      window.open(`/print/documents/${doc.id}`, "_blank", "noopener,noreferrer");
    } catch (cause) {
      onError(cause instanceof Error ? cause.message : "Unable to generate admit card");
    } finally {
      setGenerating(false);
    }
  }
      ...
      {mode === "design" ? (
        ...
        <div className="space-y-4">
          <div className="nx-card p-5">
            ...
            <p className="mt-1 text-[12.5px] text-slate-500">
              Choose a template, exam, and student to generate a printable admit card.
            </p>
            <form className="mt-5 grid gap-3 md:grid-cols-2" onSubmit={(event) => void generateAdmit(event)}>
              <label className="block">
